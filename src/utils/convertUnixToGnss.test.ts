...
import { unixAtGpsZero } from './convertGnssToUnix.ts';

describe('convertUnixToGnss', () => {
  test('to throw an error if unix timestamp is invalid', () => {
    const invalidUnixTimestamp = -1;
    expect(() => convertUnixToGnss(invalidUnixTimestamp)).toThrowError();
  });

  test('to throw an error if unix timestamp is older than GPS', () => {
    const unixTimestampOlderThanGnss = unixAtGpsZero - 1;
    expect(() => convertUnixToGnss(unixTimestampOlderThanGnss)).toThrowError();
  });
